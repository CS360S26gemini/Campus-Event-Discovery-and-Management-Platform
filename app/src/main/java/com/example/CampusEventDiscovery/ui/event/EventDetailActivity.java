...
//        btnViewTicket = findViewById(R.id.btnViewTicket);
    ...
    private void setupStaticListeners() {
        ...
        if (btnSos != null) {
            btnSos.setOnClickListener(v -> {
                long lastSent = getSharedPreferences("sos_prefs", MODE_PRIVATE)
                        .getLong("last_sent_ts", 0L);
                long elapsed = System.currentTimeMillis() - lastSent;
                if (elapsed < 60_000L) {
                    long secondsRemaining = (60_000L - elapsed) / 1000L + 1L;
                    Toast.makeText(EventDetailActivity.this,
                            "Please wait " + secondsRemaining + "s before sending another SOS.",
                            Toast.LENGTH_SHORT).show();
                    applySosCooldown(60_000L - elapsed);
                    return;
                }
                Intent sosIntent = new Intent(EventDetailActivity.this,
                        com.example.CampusEventDiscovery.ui.sos.SosActivity.class);
                sosIntent.putExtra("eventId", eventId);
                sosIntent.putExtra("eventName",
                        currentEvent != null ? currentEvent.getTitle() : "");
                sosIntent.putExtra("organizerId",
                        (currentEvent != null && currentEvent.getOrganizerId() != null)
                                ? currentEvent.getOrganizerId() : "");
                startActivity(sosIntent);
                applySosCooldown(60_000L);
            });
        }
    ...
    private void updateSosVisibility() {
        if (btnSos == null) {
            return;
        }
        if (UserRoles.isAttendee(currentUserRole) && currentUserId != null) {
    ...
    private void applySosCooldown(long remainingMs) {
